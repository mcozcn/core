...
import { updateUser, deleteUser, type User as UserType } from "@/utils/storage/users";
...
const NewUsersList = ({ users, onDeleteUser, onUserUpdated }: NewUsersListProps) => {
  ...
  const handlePermissionUpdate = async (userId: number, field: string, value: boolean) => {
    try {
            ...
            <TableRow key={user.id}>
              ...
              <TableCell>
                <div className="space-y-1">
                  <Badge variant={user.role === 'admin' ? 'default' : user.role === 'power_user' ? 'secondary' : 'outline'}>
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => 
                        handlePermissionUpdate(user.id, 'canEdit', checked as boolean)
                      }
                    />
                    <Label className="text-sm">Düzenleme</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      checked={user.canDelete}
                      onCheckedChange={(checked) => 
                        handlePermissionUpdate(user.id, 'canDelete', checked as boolean)
                      }
                    />
                    <Label className="text-sm">Silme</Label>
                  </div>
                </div>
              </TableCell>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div><strong>Rol:</strong> {user.role === 'admin' ? 'Yönetici' : user.role === 'power_user' ? 'Power User' : 'Personel'}</div>
                    </div>
                  </div>
